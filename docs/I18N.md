...
PilgrimCompare must eventually adapt to the user's location and language. For the current MVP, visible public pricing is GBP-only so server-rendered HTML and client hydration stay identical.

**Current MVP rule (2026-06-06):** Do not convert package prices from `navigator.language`, timezone, or localStorage. Multi-currency UI is future scope and must be reintroduced with a server-provided snapshot to avoid hydration mismatches.
...
### Future detection logic (infrastructure exists in `lib/i18n/region.ts`)
...
### How prices are displayed in MVP

1. Package data uses `currency`, but public UI displays deterministic GBP/en-GB formatting.
2. `getRegionSettings()` returns UK/GBP/miles by default and intentionally does not inspect browser-only values during render.
3. Browser-stored display currency values other than GBP are ignored in MVP.
4. Future multi-currency display must pass a server/client-consistent settings snapshot into SSR-ed trees.

### Future multi-currency behaviour
...
**Phase 1 (now):** GBP-only display with deterministic formatting. Conversion utilities are present but not active in public UI.
...
**Phase 3 (future):** Re-enable multi-currency display with a server-provided settings snapshot, then prioritise operators serving the user's region.
